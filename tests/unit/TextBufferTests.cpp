...
TEST_CASE("load_from_path preserves line order and strips newlines") {
    vitality::tests::TempFile temp_file("alpha\nbeta\n");
...
TEST_CASE("loading an empty file creates one empty logical line") {
    vitality::tests::TempFile temp_file("");
    const vitality::FilePath file_path = vitality::FilePath::from_command_line_arg(temp_file.path().c_str());
...
TEST_CASE("clamp_cursor fixes out of range line and column values") {
    const vitality::TextBuffer buffer = vitality::tests::load_buffer_from_contents_or_require("abcd\nef\n");
...
TEST_CASE("logical cursor helpers make logical ownership explicit without changing behavior") {
    ...
    const vitality::TextBuffer malformed = vitality::tests::load_buffer_from_contents_or_require(std::string("\xC3(", 2) + "\n");
...
TEST_CASE("horizontal movement clamps at line edges") {
    const vitality::TextBuffer buffer = vitality::tests::load_buffer_from_contents_or_require("abc\n");
...
TEST_CASE("horizontal movement is grapheme-aware on valid UTF-8 lines") {
    const vitality::TextBuffer combining = vitality::tests::load_buffer_from_contents_or_require("e\u0301x\n");
    ...
    const vitality::TextBuffer emoji = vitality::tests::load_buffer_from_contents_or_require("👍🏽a\n");
    ...
    const vitality::TextBuffer zwj = vitality::tests::load_buffer_from_contents_or_require("👩‍💻x\n");
    ...
    const vitality::TextBuffer hangul = vitality::tests::load_buffer_from_contents_or_require("하x\n");
...
TEST_CASE("preferred_column follows grapheme display semantics and falls back conservatively") {
    const vitality::TextBuffer valid = vitality::tests::load_buffer_from_contents_or_require("e\u0301x\n");
    ...
    const vitality::TextBuffer malformed = vitality::tests::load_buffer_from_contents_or_require(std::string("\x80x\n", 3));
